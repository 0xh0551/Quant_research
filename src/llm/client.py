...
class QuantLLM:
    ...
    def _record_call(*, mode: str, model: str, tier: str, cost: float,
                     usage: dict | None = None, site: str | None = None,
                     **extra: Any) -> None:
    ...
    def record_web_search(self, usage: Any, model: str, n_searches: int) -> float:
    ...
    def count_tokens(self, prompt: str, *, tier: str = "smart", system: str | None = None) -> int:
        ...
        return cast(int, self._c().messages.count_tokens(**kwargs).input_tokens)
    ...
    def batch_submit(
        ...
        *,
        ...
        sysblock = None
        if system:
            b: dict[str, Any] = {"type": "text", "text": system}
            if cache_system:
        ...
        for it in items:
            ...
            reqs.append(Request(custom_id=it["custom_id"],
        ...
        return cast(str, batch_id)

    def batch_status(self, batch_id: str) -> str:
        return cast(str, self._c().messages.batches.retrieve(batch_id).processing_status)
    ...
    def _price_for(model: str) -> tuple[float, float]:
        ...
        if model == "claude-sonnet-5" and datetime.now(UTC).date() > SONNET_INTRO_END:
    ...
    def _cost(model: str, usage: dict) -> float:
        ...
        return cast(float, round((billed_in * pin + usage["output"] * pout) / 1e6, 6))

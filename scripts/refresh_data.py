...
OKX_TOP_N = 30         # top OKX USDT swaps — صرافیِ زندهٔ Klaymen
DISCOVER_TIMEFRAMES = ["4h", "1h", "15m"]

# وایت‌لیستِ Klaymen از کانفیگِ زنده خوانده می‌شود تا جفت‌هایی که واقعاً روی OKX
...
def _klaymen_config_path() -> Path | None:
    try:
        cfg = json.loads((ROOT / "configs" / "local.json").read_text())
        user_data = cfg.get("user_data")
        return Path(user_data) / "klaymen_config.json" if user_data else None
    except Exception:
        return None


KLAYMEN_CONFIG = _klaymen_config_path()
BOOTSTRAP_DAYS = 730
...
# سرِ جایش مانده بود و دیتای okx را بی‌صدا متوقف کرده بود — در حالی که Klaymen
...
def _klaymen_whitelist_bases() -> list[str]:
    """'SOL/USDT:USDT' → 'SOLUSDT' برای جفت‌های زندهٔ Klaymen.
    ...
    """
    if KLAYMEN_CONFIG is None:
        return []
    try:
        text = KLAYMEN_CONFIG.read_text(encoding="utf-8")
        block = re.search(r'"pair_whitelist"\s*:\s*\[([^\]]*)\]', text, re.S)
        if not block:
            return []
        return [p.split(":")[0].replace("/", "") for p in re.findall(r'"([^"]+)"', block.group(1))]
    except Exception as exc:
        print(f"  WARN  could not read Klaymen whitelist ({exc})")
        return []


def discover_okx_pairs(n: int = OKX_TOP_N) -> list[tuple[str, str, str, str]]:
    """Top N OKX USDT-settled swaps by 24h volume, ∪ Klaymen's live whitelist.
    ...
    """
    forced = _klaymen_whitelist_bases()
    try:
        exchange = ccxt.okx({"options": {"defaultType": "swap"}, "enableRateLimit": True})
        tickers = exchange.fetch_tickers()
        ranked = sorted(
            [
                (sym, float(t.get("baseVolume") or 0) * float(t.get("last") or 0))
                for sym, t in tickers.items()
        ...
        print(f"  WARN  OKX discovery failed ({exc}); falling back to Klaymen whitelist only")
        bases = []

    # جفت‌های زندهٔ Klaymen همیشه وارد می‌شوند، حتی اگر خارج از top-N باشند
    for b in forced:
        if b not in bases:
            bases.append(b)
    result = [("okx", "futures", b, tf) for b in bases for tf in DISCOVER_TIMEFRAMES]
    print(f"  okx discovered {len(bases)} pairs ({len(forced)} از وایت‌لیست Klaymen) "
...
def main() -> int:
    ...
    print("discovering OKX top USDT swaps (venue زندهٔ Klaymen)…")

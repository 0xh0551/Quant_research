#!/usr/bin/env python3
"""رفرشِ بات‌های «بریجِ کوانتِ پرتکرار» (Mickey روی gate، Wall_E روی hyperliquid).
...
هر بات روی صرافی و نمادهای متفاوت ⇒ تنوعِ واقعی (Mickey: gate/USDT،
Wall_E: hyperliquid/USDC).
...
#   • Mickey → MickeyXSMN  (bybit، FreqAI μ/α، ۲۹ جفت)        — strategies/MickeyXSMN.py
#   • Wall_E → WalleXSMN   (hyperliquid، XS momentumِ قاعده‌محور، ۳۴ جفت) — strategies/WalleXSMN.py
...
_LEGACY_BOTS_DISABLED: dict[str, dict] = {
    "Mickey": {
        ...
        "manifest": UD / "wf_candidates_mickey.json",
        "config": UD / "mickey_config.json",
        "env": NOCHES / "mickey.env",
        "top_n": 8, "min_vol": 3_000_000.0,
    },
    "Wall_E": {
        "container": "Wall_E",
        "exchange": "hyperliquid", "quote": "USDC", "swap_opt": False,
        "manifest": UD / "wf_candidates_walle.json",
        "config": UD / "walle_config.json",
        "env": NOCHES / "walle.env",

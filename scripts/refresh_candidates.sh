...
#   3) ری‌استارتِ bot5/bot6 فقط اگر پلنِ زندهٔ جفت‌هایشان عوض شده باشد
#   4) چرخش جفت‌ارز bot3/bot4 از روی امتیاز سازگاری RL/ML (با هیسترزیس)
...

{
  ...
  OK=true; FAILED=null
  st_run "wf_scan"
  if ! "$PY" scripts/refresh_candidates.py --reload-bot5 --reload-bot6 "$@"; then
    OK=false; FAILED="\"wf_scan\""
    echo "FAIL: pipeline failed at step wf_scan"
  fi
  # چرخش جفت‌ارز بات‌های RL/ML (bot3/bot4) — هیسترزیس + ری‌استارت فقط در صورت تعویض

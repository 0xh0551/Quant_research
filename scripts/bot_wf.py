#!/usr/bin/env python3
"""Walk-forward validation for bot1's μ/α regressor (leakage-free, cost-aware).
...
ویژگی‌های bot1ML را روی دادهٔ واقعیِ Gate بازسازی می‌کند و با walk-forwardِ
...
هدفِ μ دقیقاً مطابق bot1ML.set_freqai_targets:
  بازده آتیِ افق h  →  بازار-خنثی (حذفِ بتای trailing نسبت‌به‌BTC)  →  مقیاس‌شده با
  نوسانِ افق (کمیتِ شبه-شارپ)، clip ±4.

استفاده:
  .venv/bin/python scripts/bot1_wf.py --pair ETH --tf 1h --features reduced
  .venv/bin/python scripts/bot1_wf.py --pair ETH,HYPE,WLD,ZEC --features full --train-min 1080
"""
...
# ── هدفِ μ/α (دقیقاً مطابق bot1ML.set_freqai_targets) ─────────────────────────
...
def main() -> int:
    ...
    if rows:
        ...
        print(f"\n=== bot1 WF — tf={args.tf} features={args.features} "
